...
async function loadAllUsersAndChurches() {
    try {
        ...
        const adminCount = allUsersData.filter(p => p.role === 'admin' || p.role === 'super_admin').length;
...
function renderUsersTable(users) {
    ...
    tbody.innerHTML = users.map(user => {
        ...
        const roleBadgeColor = user.role === 'super_admin' ? 'bg-red-100 text-red-800 border border-red-300' : (user.role === 'admin' ? 'bg-purple-100 text-purple-800' : (user.role === 'finance' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'));
